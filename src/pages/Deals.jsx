  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 fade-in">
      <h1 className="text-3xl font-extrabold flex items-center gap-2">
        <TagIcon className="w-8 h-8 text-rose-500" /> Live deals & coupons
      </h1>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {cats.map((c) => (
          <button
            ...
            onClick={() => setCat(c.id)}
            className={
              "chip px-4 py-2 rounded-full text-sm font-semibold " +
              (cat === c.id
      ...
      {loading ? (
        ...
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {deals.length === 0 && (
            <p className="text-slate-500 col-span-full">No active deals found.</p>
          )}
        </div>
      )}
    </div>
