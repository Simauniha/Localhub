...
import { StarIcon, MapPinIcon } from "../icons/index.jsx";
  ...
  return (
    <Link to={`/listings/${item.id}`} className="card-hover block bg-white dark:bg-slate-800 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-700">
      <div className="relative">
        ...
        <span className="absolute top-3 left-3 bg-white/90 backdrop-blur text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1 text-slate-800">
          <StarIcon className="w-3.5 h-3.5 text-amber-500" /> {item.rating}
        </span>
      </div>
      <div className="p-4">
        ...
        <div className="text-xs text-slate-500 mt-1 flex items-center gap-1"><MapPinIcon className="w-3.5 h-3.5" /> {item.area}</div>
